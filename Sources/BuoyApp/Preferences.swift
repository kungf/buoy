...
/// - Selection: key `buoy.selectedProviderIds`, value `[String]` (ordered providerId array).
/// - Ball positions: key `buoy.ballPositions`, value `[String: [Double]]` where each pair is `[x, y]`.
struct Preferences: SelectionStorage, BallPositionStorage {
    private static let selectionKey = "buoy.selectedProviderIds"
    private static let positionsKey = "buoy.ballPositions"
