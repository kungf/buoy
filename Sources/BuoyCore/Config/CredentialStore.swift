...
/// ~/.buoy/config.json 的单 provider 条目。字段按 auth 模式取用：
...
public struct BuoyConfigFile: Codable, Sendable, Equatable {
...
public enum CredentialStore {
    public static var defaultURL: URL {
        FileManager.default.homeDirectoryForCurrentUser
            .appendingPathComponent(".buoy/config.json")
    }

    public static func load(from url: URL = defaultURL) -> BuoyConfigFile? {
        guard let data = try? Data(contentsOf: url) else { return nil }
        return try? JSONDecoder().decode(BuoyConfigFile.self, from: data)
    }

    /// 条目 → 统一 Credential。ak/sk 齐备优先 volcAccessKey，其次 bearer token。
    public static func credential(for providerId: String, from config: BuoyConfigFile?) -> Credential? {
