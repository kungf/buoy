...
| M1 轮询接真数据 | `UsageStore` 已从 `CredentialStore.load()` 拉真数据并发轮询（非仅 mock）；`trwyctl` 联调 harness | `UsageStore.swift:50-82` |
...
- 凭证存 `~/.trwy/config.json`（明文 JSON，chmod 600）--DESIGN §10 要求 **Keychain**，M0 doc 自标注 "M2 迁 Keychain"。
...
| 13 | **测试覆盖** | 21 个全 TokenRunwayCore 单测；无 UI 测试、无 adapter 集成（recorded fixture）。真联调靠 `trwyctl` 手动 | §11 |
...
优先级排序的依据是 TokenRunway 的核心价值主张--**"预测优于报数"** 与 **"5h 额度 10 分钟烧完才发现"** 的痛点。已就绪但未接线的预测能力（缺口 #1）是最高 ROI：代码大半写好了，只差接线。

### Phase 1 - 接通预测（M2 核心，最高优先级）✅ 完成（2026-07-28）

把 `BurnRateEstimator` 从死代码变成球面上的呼吸节奏与 ETA。这是 TokenRunway 区别于竞品的命门。

- [x] **1.1 采样收集**：新增 `ForecastEngine`（TokenRunwayCore），每次成功拉取对每个 quota 追加 `UsageSample`，内存环形 buffer（cap 120）。
...
- [x] **2.3 持久化**：`CacheStore` 存 `~/.trwy/cache.json`（`TokenRunwayCache = reports + ForecastEngine`）；启动加载，每次成功拉取后保存。重启不冷启动。
- [x] **2.4 DeepSeek 跳崖识别**：Phase 1 已用 `-remaining` 代理量 + reset 启发式实现（充值回升 -> 断段重置基线）。

新增 `BackoffPolicy` / `CacheStore` / `TokenRunwayCache`（TokenRunwayCore Store/），`ForecastEngine`/`BurnRateEstimator`/`UsageSample` 加 `Codable`。`swift test` 33/33 ✅。

### Phase 3 - Keychain 迁移（安全，M2 并行）

- [ ] **3.1 SecretStore**：`SecItemAdd/Update/Copy` 薄封装，`kSecClassGenericPassword`，`account = providerId`。
- [ ] **3.2 迁移**：首次启动读 `config.json` -> 写 Keychain -> 删除明文文件；`trwyctl` 凭证来源改为 Keychain。
...
- [ ] **7.4 adapter 集成测试**（#13）：recorded fixture，离线回放，替代手动 `trwyctl` 联调。
...
**MVP 收敛点**：Phase 1+2+3+4 完成即达成 DESIGN 的 M2 全量（预测 + 预警 + 安全 + 持久化），此时 TokenRunway 已对用户产生核心价值--可考虑先发一个内部测试版，再用 Phase 5-8 打磨上线。
