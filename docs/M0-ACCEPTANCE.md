...
浮标球 + 总面板已在真机跑通（mock 数据），TokenRunwayCore 16/16 单测通过，DeepSeek 适配器真联调成功。

## 交付物

| 模块 | 内容 |
|---|---|
| SPM 工程 | TokenRunwayCore（Foundation-only）/ TokenRunwayApp / trwyctl / TokenRunwayCoreTests 四 target |
| TokenRunwayCore | 统一 Quota 模型、燃烧率/ETA、HealthScore、HTTP 层 |
...
| 打包 | `Scripts/make-app.sh` → build/TokenRunway.app（LSUIElement、com.wyang.tokenrunway） |

## 验证记录

- `swift build` ✅ / `swift test` 16/16 ✅（含 V4 签名参考向量，Python hashlib/hmac 独立实现交叉验证）
- **DeepSeek 真联调**：`trwyctl deepseek` → 余额 ¥1.25（granted 0 / topped_up 1.25）✅
...
2. 跑：`TRWY_VOLC_AK=xxx TRWY_VOLC_SK=yyy .build/debug/trwyctl volcano`
...
TokenRunway.app 仍在运行（右上角球，mock 数据）。退出：`pkill -x TokenRunway`。

## 补充（2026-07-28 晚）：凭证配置落地

- 凭证已写入 `~/.trwy/config.json`（chmod 600，**仓库外**，git 不可达）：DeepSeek token + 火山 ark- key/baseURL
- `trwyctl` 凭证优先级：env > ~/.trwy/config.json；DeepSeek 已从配置加载并联调通过
...
- **拿到 AK/SK 后**填入 `~/.trwy/config.json` 的 `providers.volcano.ak/sk` 即可，无需改代码
...
`trwyctl volcano` 实测输出（medium 套餐）：
